...
Agent X 是未来多轮迭代的主控调度角色，不直接替代 Agent A、Agent B 或 Agent C。

Agent X 接收人工总目标 X 后，必须完成：

1. 阅读必读文件、当前工作树、git 状态和已有 Agent 结果。
2. 将总目标拆成可验收的小轮次目标，明确本轮目标、非目标、风险和停止条件。
3. 每轮按 `Agent A -> Agent B -> Agent C` 顺序推进。
4. 要求 Agent A 为每轮生成版本化提示词。
5. 等 Agent B 完成本地轻量检查、版本 commit 和 `git push origin main`。
6. 等 Agent C 下载并核对最新 GitHub Actions 未加密 artifact。
7. 根据 Agent C 结论判断继续下一轮、退回 Agent B 修复、暂停等待人工确认，或宣布总目标完成。

Agent X 循环边界：

- Agent X 只能调度和判断，不得跳过 Agent A 的提示词、Agent B 的实现 push、Agent C 的云端 artifact 验收。
- Agent X 继续下一轮的依据必须是 Agent C 对 `origin/main` 最新 commit、run id、run attempt 和 artifact 的明确结论。
- Agent C 不通过时，Agent X 只能退回 Agent B 在 `main` 上追加修复 commit，或暂停等待人工决策。
- Agent X 不得为了推进循环扩大无关改动范围。
...
- 总目标状态：继续、退回、暂停或完成。
- 已完成轮次、当前轮次版本号和下一轮建议。
- 最近一次 Agent C 结论、commit SHA、run id、run attempt 和 artifact 名称。
- 若暂停或退回，必须列出阻塞原因、需要人工确认的问题或 Agent B 修复要求。
...
Agent X 额外输出：总目标状态、轮次摘要、最近一次 Agent C artifact 核对摘要、继续/退回/暂停/完成判断，以及下一轮具体建议。
...
- 禁止 Agent X 把旧 run、旧 artifact 或本地输出冒充最新云端结果。
