...
- Agent C 退回修复：CI 现在把原始 video dependency manifest 复制到 `ci-results/video-native-dependency-manifest.json` 并做存在性检查；preflight 只接受 repo 内真实 evidence path，拒绝绝对路径、`..` 逃逸和伪造非空证据；contract probe 输出稳定 unavailable/error/cancellation/native/output 摘要。
...
- 核心变更：
  ...
  - Agent C 退回目录级 volume 校验后，stage/restore/finalize 改为按文件存在四态优先校验 actual original/staged payload identity；双方存在时还直接比较两个 actual 文件，只有缺失端才使用安全目录 fallback，sourceMissing 不读取不存在文件。
  - production move catch 读取 original/staged 四态：仅 original 保留为普通失败，仅 staged 与两处均缺失都保留原 staged token 并要求恢复；两处都有时保留两边和原 token、进入阻塞的 `restorePending`，Retry Restore 在冲突仍在时不覆盖，外部状态收敛为 original-only 或 staged-only 后再安全完成。move 前 source missing 仍是独立 no-payload disposition。
...
- 验证结果：生产 `AppFileStore` `/private/tmp` 真实文件 probe 已通过 bytes、正常 lifecycle/幂等、sourceMissing、staging 内外 symlink、真实同卷 identity、注入目录 identity 缺失/跨卷、move error 四态，以及 stage/restore/finalize 的 actual original/staged payload identity mismatch 和 sourceMissing 安全目录 fallback；固定 `git diff --check`、plist、workflow YAML、普通/native 12 文件 Swift parse、额外 12 文件 iOS typecheck，以及追加 conflict catch 位于 generic catch 前且保留 token/阻塞 phase 的 source-contract 检查均退出 0。完整 iPhoneOS build 与 native preflight 交给 push 后 GitHub Actions，artifact 待 Agent C 下载核对。
...
- 日期：2026-07-26
- 核心变更：
  - 共享 `SciFiStatusPill` 的普通 Dynamic Type 分支改为 `ViewThatFits(in: .horizontal)`。
  - 横向候选使用固有宽度参与布局，空间足够时保留既有横排 `Label`，不足时回退为纵向 icon + 最多两行 title。
  - accessibility Dynamic Type 直接使用纵向 icon + 可换行 title。
  - 保留颜色、capsule 背景/描边、padding、组合 accessibility 语义和所有调用方业务状态不变。
- 关键文件：
...
- 遗留事项：本轮不做 simulator / iPad Stage Manager / Split View / Mac window / VoiceOver 实机截图目检，不修改 status pill 调用方状态、generation gate、model/prompt data、SwiftData schema、file storage、native backend、StoreKit、Mac Catalyst、Xcode project、workflow 或测试规范。
...
- 日期：2026-07-12
- 核心变更：
  - `PlanView.planOverview` 普通 Dynamic Type 的 icon + copy 分支改为 `ViewThatFits(in: .horizontal)`。
  - 优先横排 overview icon/copy，横向空间不足时回退纵向堆叠。
  - accessibility Dynamic Type 现有纵向布局保持不变。
  - 保留 Local plan 边界说明、StoreKit 未配置/paid candidates planning-only 文案、panel 样式和 VoiceOver label/value/hint 不变。
- 关键文件：
  - `LocalDiffusion/Views/RootContentView.swift`
  - `README.md`
  - `md/flow/flow.md`
...
- 日期：2026-07-12
- 核心变更：
  - `GenerationView.consoleStatusPills` 的普通 Dynamic Type 分支改为 `ViewThatFits(in: .horizontal)`。
  - 优先横排 backend + model status pills，横向空间不足时回退纵向堆叠。
  - accessibility Dynamic Type 现有纵向布局保持不变。
  - 保留组合 VoiceOver label/value/hint、pill 内容、generation gate 和 run/result 行为不变。
- 关键文件：
  - `LocalDiffusion/Views/Generation/GenerationView.swift`
  - `README.md`
  - `md/flow/flow.md`
  - `md/flow/flowchart.md`
  - `md/prompt/v1（体验优化）/v1.146（Generate控制台状态Pills窄宽度回退）.md`
  - `update_log.md`
- 验证结果：需要运行本地 `git diff --check`、`plutil`、workflow YAML 解析、普通 Swift parse、native bridge Swift parse；GitHub Actions 结果包由 Agent C 下载核对。
- 遗留事项：本轮不做 simulator / iPad Stage Manager / Split View / VoiceOver 实机目检，不修改 consoleOverview、ParameterEditor、generation gate、run/result、SwiftData schema、file storage、native backend、StoreKit、Mac Catalyst、Xcode project、workflow 或测试规范。

### v1.145 / Generate 控制台概览窄宽度回退

- 日期：2026-07-12
- 核心变更：
  - `GenerationView.consoleOverview` 的普通 Dynamic Type 分支改为 `ViewThatFits(in: .horizontal)`。
  - 优先横排 icon + copy，横向空间不足时回退纵向堆叠。
  - accessibility Dynamic Type 现有纵向布局保持不变。
  - 保留控制台文案、icon、status pills、generation gate 和 run/result 行为不变。
- 关键文件：
  - `LocalDiffusion/Views/Generation/GenerationView.swift`
  - `README.md`
  - `md/flow/flow.md`
  - `md/flow/flowchart.md`
  - `md/prompt/v1（体验优化）/v1.145（Generate控制台概览窄宽度回退）.md`
  - `update_log.md`
- 验证结果：需要运行本地 `git diff --check`、`plutil`、workflow YAML 解析、普通 Swift parse、native bridge Swift parse；GitHub Actions 结果包由 Agent C 下载核对。
- 遗留事项：本轮不做 simulator / iPad Stage Manager / Split View / VoiceOver 实机目检，不修改 consoleStatusPills、ParameterEditor、generation gate、run/result、SwiftData schema、file storage、native backend、StoreKit、Mac Catalyst、Xcode project、workflow 或测试规范。

### v1.144 / Generate Prompt 元数据窄宽度回退

- 日期：2026-07-12
- 核心变更：
  - `GenerationView.promptEditorMetadata` 的普通 Dynamic Type 分支改为 `ViewThatFits(in: .horizontal)`。
  - 优先横排 character count + clear，横向空间不足时回退纵向堆叠。
  - accessibility Dynamic Type 现有纵向布局保持不变。
  - 补齐 `flowchart.md` GENUI 摘要中的 Generate prompt header/metadata 窄宽度回退描述。
  - 保留 clear button 行为、character count accessibility、prompt editing、generation gate 和 run/result 行为不变。
- 关键文件：
  - `LocalDiffusion/Views/Generation/GenerationView.swift`
  - `README.md`
  - `md/flow/flow.md`
  - `md/flow/flowchart.md`
  - `md/prompt/v1（体验优化）/v1.144（GeneratePrompt元数据窄宽度回退）.md`
  - `update_log.md`
- 验证结果：需要运行本地 `git diff --check`、`plutil`、workflow YAML 解析、普通 Swift parse、native bridge Swift parse；GitHub Actions 结果包由 Agent C 下载核对。
- 遗留事项：本轮不做 simulator / iPad Stage Manager / Split View / VoiceOver 实机目检，不修改 ParameterEditor、generation gate、run/result、SwiftData schema、file storage、native backend、StoreKit、Mac Catalyst、Xcode project、workflow 或测试规范。

### v1.143 / Generate Prompt 编辑器标题窄宽度回退

- 日期：2026-07-12
- 核心变更：
  - `GenerationView.promptEditorHeader` 的普通 Dynamic Type 分支改为 `ViewThatFits(in: .horizontal)`。
  - 优先横排 title + metadata（character count/clear），横向空间不足时回退纵向堆叠。
  - accessibility Dynamic Type 现有纵向布局保持不变。
  - 保留 prompt editing、character count accessibility、clear button、generation gate 和 run/result 行为不变。
- 关键文件：
  - `LocalDiffusion/Views/Generation/GenerationView.swift`
  - `README.md`
  - `md/flow/flow.md`
...
- 日期：2026-07-12
- 核心变更：
  - `PromptTemplateEditor.promptEditor` 普通 Dynamic Type 的 title/character-count header 改为 `ViewThatFits(in: .horizontal)`。
  - 优先横排 title + count，横向空间不足时回退纵向堆叠。
  - accessibility Dynamic Type 现有纵向布局保持不变。
  - 保留 TextEditor、placeholder、character count accessibility、Save/Cancel 行为不变。
- 关键文件：
  - `LocalDiffusion/Views/Prompts/PromptLibraryView.swift`
  - `README.md`
  - `md/flow/flow.md`
  ...
  - `update_log.md`
- 验证结果：需要运行本地 `git diff --check`、`plutil`、workflow YAML 解析、普通 Swift parse、native bridge Swift parse；GitHub Actions 结果包由 Agent C 下载核对。
- 遗留事项：本轮不做 simulator / iPad Stage Manager / Split View / VoiceOver 实机目检，不修改 Generate prompt editor、ParameterEditor、category editor、SwiftData schema、file storage、native backend、StoreKit、Mac Catalyst、Xcode project、workflow 或测试规范。

### v1.141 / Prompt 分类标题窄宽度回退

- 日期：2026-07-12
- 核心变更：
  - `PromptCategoryHeader.headerContent` 的普通 Dynamic Type 分支改为 `ViewThatFits(in: .horizontal)`。
  - 优先横排 category title + actions menu，横向空间不足时回退纵向堆叠。
  - accessibility Dynamic Type 现有纵向布局保持不变。
  - 保留 uncategorized 不显示 actions、rename/clear 行为、heading/count accessibility 语义不变。
- 关键文件：
  - `LocalDiffusion/Views/Prompts/PromptLibraryView.swift`
  - `README.md`
  - `md/flow/flow.md`
  ...
  - `update_log.md`
- 验证结果：需要运行本地 `git diff --check`、`plutil`、workflow YAML 解析、普通 Swift parse、native bridge Swift parse；GitHub Actions 结果包由 Agent C 下载核对。
- 遗留事项：本轮不做 simulator / iPad Stage Manager / Split View / VoiceOver 实机目检，不修改 template row 业务、editor、search、ParameterEditor、SwiftData schema、file storage、native backend、StoreKit、Mac Catalyst、Xcode project、workflow 或测试规范。

### v1.140 / Prompt 模板行标题窄宽度回退

- 日期：2026-07-12
- 核心变更：
  - `PromptTemplateRow.header` 的普通 Dynamic Type 分支改为 `ViewThatFits(in: .horizontal)`。
  - 优先横排 templateCopy + Edit/Load actions，横向空间不足时回退纵向堆叠。
  - accessibility Dynamic Type 现有纵向布局保持不变。
  - 保留 metric pills、row-level accessibility summary、Edit/Load 行为、文案和 panel 样式不变。
- 关键文件：
  - `LocalDiffusion/Views/Prompts/PromptLibraryView.swift`
  - `README.md`
  - `md/flow/flow.md`
  ...
  - `update_log.md`
- 验证结果：需要运行本地 `git diff --check`、`plutil`、workflow YAML 解析、普通 Swift parse、native bridge Swift parse；GitHub Actions 结果包由 Agent C 下载核对。
- 遗留事项：本轮不做 simulator / iPad Stage Manager / Split View / VoiceOver 实机目检，不修改 category header、template editor、search、ParameterEditor、SwiftData schema、file storage、native backend、StoreKit、Mac Catalyst、Xcode project、workflow 或测试规范。

### v1.139 / Models 存储矩阵标题窄宽度回退

- 日期：2026-07-12
- 核心变更：
  - `StorageSummaryRow.header` 的普通 Dynamic Type 分支改为 `ViewThatFits(in: .horizontal)`。
  - 优先横排 titleBlock + ready pill，横向空间不足时回退纵向堆叠。
  - accessibility Dynamic Type 现有纵向布局保持不变。
  - 保留 storage metrics、accessibility summary、ready pill 文案/颜色和 panel 样式不变。
- 关键文件：
  - `LocalDiffusion/Views/Models/ModelLibraryView.swift`
  - `README.md`
  - `md/flow/flow.md`
  ...
  - `update_log.md`
- 验证结果：需要运行本地 `git diff --check`、`plutil`、workflow YAML 解析、普通 Swift parse、native bridge Swift parse；GitHub Actions 结果包由 Agent C 下载核对。
- 遗留事项：本轮不做 simulator / iPad Stage Manager / Split View / VoiceOver 实机目检，不修改 tracked/untracked rows 业务、download manager、Add Model sheet、detail sheet、SwiftData schema、file storage、native backend、StoreKit、Mac Catalyst、Xcode project、workflow 或测试规范。

### v1.138 / Models 未跟踪行窄宽度回退

- 日期：2026-07-12
- 核心变更：
  - `UntrackedModelFileRow.rowContent` 的普通 Dynamic Type 分支改为 `ViewThatFits(in: .horizontal)`。
  - 优先横排 fileInfo + Import/Delete actions，横向空间不足时回退纵向堆叠。
  - accessibility Dynamic Type 现有纵向布局保持不变。
  - 保留 import/delete 行为、文件名/大小展示、accessibility label/value/hint 和 panel 样式不变。
- 关键文件：
  - `LocalDiffusion/Views/Models/ModelLibraryView.swift`
  - `README.md`
  - `md/flow/flow.md`
  - `md/flow/flowchart.md`
  - `md/prompt/v1（体验优化）/v1.138（Models未跟踪行窄宽度回退）.md`
  - `update_log.md`
- 验证结果：需要运行本地 `git diff --check`、`plutil`、workflow YAML 解析、普通 Swift parse、native bridge Swift parse；GitHub Actions 结果包由 Agent C 下载核对。
- 遗留事项：本轮不做 simulator / iPad Stage Manager / Split View / VoiceOver 实机目检，不修改 tracked ModelRow 业务、download manager、Add Model sheet、detail sheet、SwiftData schema、file storage、native backend、StoreKit、Mac Catalyst、Xcode project、workflow 或测试规范。

### v1.137 / Models 行底部窄宽度回退

- 日期：2026-07-12
- 核心变更：
  - `ModelRow.footer` 的普通 Dynamic Type 分支改为 `ViewThatFits(in: .horizontal)`。
  - 优先横排 size label + controls，横向空间不足时回退纵向堆叠。
  - accessibility Dynamic Type 现有纵向布局保持不变。
  - 保留 header、progress、controls 行为、下载/暂停/取消/删除/详情和 panel 样式不变。
- 关键文件：
  - `LocalDiffusion/Views/Models/ModelLibraryView.swift`
  - `README.md`
  - `md/flow/flow.md`
  ...
  - `update_log.md`
- 验证结果：需要运行本地 `git diff --check`、`plutil`、workflow YAML 解析、普通 Swift parse、native bridge Swift parse；GitHub Actions 结果包由 Agent C 下载核对。
- 遗留事项：本轮不做 simulator / iPad Stage Manager / Split View / VoiceOver 实机目检，不修改 download manager、Add Model sheet、detail sheet、untracked import、SwiftData schema、file storage、native backend、StoreKit、Mac Catalyst、Xcode project、workflow 或测试规范。

### v1.136 / Models 行标题窄宽度回退

- 日期：2026-07-12
- 核心变更：
  - `ModelRow.header` 的普通 Dynamic Type 分支改为 `ViewThatFits(in: .horizontal)`。
  - 优先横排 model identity + status pill，横向空间不足时回退纵向堆叠。
  - accessibility Dynamic Type 现有纵向布局保持不变。
  - 保留 progress、footer controls、status 计算、下载/暂停/取消/删除/详情行为和 panel 样式不变。
- 关键文件：
  - `LocalDiffusion/Views/Models/ModelLibraryView.swift`
  - `README.md`
  - `md/flow/flow.md`
...
- 日期：2026-07-12
- 核心变更：
  - `PromptTemplateRow.metricPills` 的普通 Dynamic Type 分支改为 `ViewThatFits(in: .horizontal)`。
  - 优先横排 steps/sampler/size metric pills，横向空间不足时回退纵向堆叠。
  - accessibility Dynamic Type 现有纵向布局保持不变。
  - 保留 Edit/Load 控件、row-level accessibility summary、pill 文案/图标/颜色/label/value/hint、模板加载/编辑业务和 panel 样式不变。
- 关键文件：
  - `LocalDiffusion/Views/Prompts/PromptLibraryView.swift`
  - `README.md`
  - `md/flow/flow.md`
  ...
  - `update_log.md`
- 验证结果：需要运行本地 `git diff --check`、`plutil`、workflow YAML 解析、普通 Swift parse、native bridge Swift parse；GitHub Actions 结果包由 Agent C 下载核对。
- 遗留事项：本轮不做 simulator / iPad Stage Manager / Split View / VoiceOver 实机目检，不修改 template CRUD、generation load path、SwiftData schema、file storage、native backend、StoreKit、Mac Catalyst、Xcode project、workflow 或测试规范。

### v1.134 / Plan 说明行无障碍回退

- 日期：2026-07-07
- 核心变更：
  - `PlanNoteRow` 读取 `dynamicTypeSize`。
  - 普通 Dynamic Type 下保留既有横向 `Label` 结构。
  - accessibility Dynamic Type 下回退为纵向 icon + note text，减少长 StoreKit/Mac 说明被挤成窄文本柱的风险。
  - 保留 note 文案、system image、icon color、frame、accessibility label/value/hint、StoreKit 未配置边界、paid candidates planning-only 状态、Mac/Catalyst 未启用说明和所有业务逻辑不变。
- 关键文件：
  - `LocalDiffusion/Views/RootContentView.swift`
  - `README.md`
  - `md/flow/flow.md`
...
- 日期：2026-07-07
- 核心变更：
  - `PlanStatusSummaryRow` 读取 `dynamicTypeSize`，普通 Dynamic Type 下使用 `ViewThatFits(in: .horizontal)`。
  - Plan Current Build 和 Platform Status summary rows 现在优先横向显示标题和状态徽章，横向空间不足时回退为既有纵向布局。
  - accessibility Dynamic Type 继续直接使用纵向布局，避免大字号横向挤压。
  - `PlanStatusBadge` 增加默认保持全宽的 `fillsWidth` 参数，让横向摘要行可以使用非全宽徽章，其他状态行保持原样。
  - 保留 Plan 可见文案、状态 token、VoiceOver label/value/hint、StoreKit 未配置边界、paid candidates planning-only 状态、Mac/Catalyst 未启用说明和所有业务逻辑不变。
- 关键文件：
  - `LocalDiffusion/Views/RootContentView.swift`
  - `README.md`
  - `md/flow/flow.md`
...
- 日期：2026-07-07
- 核心变更：
  - `GenerationView.resultSection` 中 `View in Gallery` 按钮增加 `.frame(minHeight: 44)`。
  - Generate Result 区 Gallery handoff 操作现在显式保持 44pt 最小命中区，和 Run 区关键操作的触控约束保持一致。
  - 保留 `onShowGallery()` action、`latestGeneratedImageID == nil` disabled 条件、button style、可见文案、辅助功能 label/value/hint、result image preview、Gallery handoff 语义和所有生成逻辑不变。
- 关键文件：
  - `LocalDiffusion/Views/Generation/GenerationView.swift`
  - `README.md`
  - `md/flow/flow.md`
  ...
  - `update_log.md`
- 验证结果：需要运行本地 `git diff --check`、`plutil`、workflow YAML 解析、普通 Swift parse、native bridge Swift parse；GitHub Actions 结果包由 Agent C 下载核对。
- 遗留事项：本轮不做 simulator Split View / Stage Manager / VoiceOver 触控验证，不修改 button style、button action、disabled 状态、accessibility 文案、result image preview、Gallery navigation、generation save flow、SwiftData schema、file storage、native backend、StoreKit、Mac Catalyst、Xcode project、workflow 或测试规范。
...
- 日期：2026-07-07
- 核心变更：
  - `GenerationView.runSection` 中 `ProgressView` label 的 `Text(viewModel.progress.stage)` 增加 `.fixedSize(horizontal: false, vertical: true)`。
  - Generate 正在生成时的可见进度阶段文本现在可在窄宽度或较长阶段文案下垂直展开。
  - 保留 `ProgressView(value:)`、progress fraction、tint、辅助功能 label/value/hint、取消按钮、generation gate 和所有生成逻辑不变。
- 关键文件：
  - `LocalDiffusion/Views/Generation/GenerationView.swift`
  - `README.md`
  - `md/flow/flow.md`
  ...
  - `update_log.md`
- 验证结果：需要运行本地 `git diff --check`、`plutil`、workflow YAML 解析、普通 Swift parse、native bridge Swift parse；GitHub Actions 结果包由 Agent C 下载核对。
- 遗留事项：本轮不做 simulator Split View / Stage Manager / 正在生成状态截图目检，不修改 progress fraction、cancel flow、generation task、native callback、generation gate、model selection、prompt editing、ParameterEditor、run/progress/result 逻辑、Gallery handoff、SwiftData schema、file storage、native backend、StoreKit、Mac Catalyst、Xcode project、workflow 或测试规范。
...
- 日期：2026-07-07
- 核心变更：
  - `ParameterEditor.parameterHeader(title:value:)` 的普通 Dynamic Type 分支改为使用 `ViewThatFits(in: .horizontal)`。
  - CFG header 现在优先横排显示 title/value，横向空间不足时回退纵向。
  - accessibility Dynamic Type 仍直接使用纵向布局。
  - 保留 CFG slider、`parameters.cfgScale` 绑定、数值格式、辅助功能语义和所有参数值逻辑不变。
- 关键文件：
  - `LocalDiffusion/Views/Shared/ParameterEditor.swift`
  - `README.md`
  - `md/flow/flow.md`
  ...
  - `update_log.md`
- 验证结果：需要运行本地 `git diff --check`、`plutil`、workflow YAML 解析、普通 Swift parse、native bridge Swift parse；GitHub Actions 结果包由 Agent C 下载核对。
- 遗留事项：本轮不做 simulator Split View / Stage Manager 截图目检，不修改 Seed、Randomize Seed、Size preset、steps、width、height、sampler、generation gate、model selection、prompt editing、parameter values、generation save flow、Gallery handoff、SwiftData schema、file storage、native backend、StoreKit、Mac Catalyst、Xcode project、workflow 或测试规范。
...
- 日期：2026-07-07
- 核心变更：
  - `PlanView.platformStatusContent` 中 `Mac support status` note 的 text 明确 Mac support 仍是 planned，当前 iPhone/iPad app 可用、Mac/Catalyst app 未启用。
  - 同一 note 明确当前没有 separate Mac binary、Mac signing profile、sandbox entitlement 或 notarization pipeline。
  - 同步 accessibility hint，让 VoiceOver 用户听到当前没有这些 Mac 发行流水线产物或有效 Mac distribution path。
  - 保留 Platform Status summary rows、note label、system image、icon color、排序、row renderer、Mac Readiness、Availability 和所有业务逻辑不变。
- 关键文件：
  - `LocalDiffusion/Views/RootContentView.swift`
  - `README.md`
  - `md/flow/flow.md`
