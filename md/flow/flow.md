...
- Models：下载、导入、暂停、恢复、删除、检查未追踪模型文件；行内控制按钮提供可访问文本 label 和 44pt 命中区，storage/model/untracked rows 在 accessibility Dynamic Type 下改为纵向堆叠以保持可读，模型行和详情状态消息使用可换行 message row 并提供明确辅助功能 label/value/hint。
...
- Generate：主要生成入口。
- Models：模型下载、导入、删除、状态恢复入口，行内控制、列表状态消息和详情状态消息支持可访问 label/value/hint 与大字号可读布局。
