...
- Prompts：保存、分类、编辑、加载模板；分类菜单、模板编辑和加载控件提供可访问文本 label 和 44pt 命中区，模板行和共享参数编辑控件在 accessibility Dynamic Type 下纵向堆叠以保持可读。
...
- Gallery：生成结果查看、过滤、复用入口，图块支持大字号可读元数据和 VoiceOver 摘要。
- Prompts：提示词模板维护入口，分类/模板行控制和共享参数编辑支持可访问 label 和大字号堆叠。
