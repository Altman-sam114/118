...
- Models：下载、导入、暂停、恢复、删除、检查未追踪模型文件；toolbar Refresh Storage 暴露模型目录和 tracked/untracked GGUF 核对语义，toolbar Add menu 暴露 Hugging Face 下载、本地 GGUF 导入入口和当前 import 状态；空状态暴露 no tracked models/no untracked GGUF files 和 Download/Import 下一步入口语义；Add Model 支持 URL 解析、完成字段的键盘提交，以及 URL、Display name、Repository、GGUF file path/direct URL、Revision、Family 字段的 label/value/hint 语义，Download 按钮提供 ready/missing VoiceOver 状态；行内控制按钮和详情 sheet 操作按钮提供可访问文本 label、模型名上下文和 44pt 命中区，tracked model 删除确认按钮暴露单模型名称/多模型数量与确认/取消语义，详情 Native Loading mode 和 CLIP/T5/VAE auxiliary model pickers 提供当前值、模型上下文和用途 hint，storage/model/untracked rows 在 accessibility Dynamic Type 下改为纵向堆叠以保持可读，未跟踪文件行和 Import/Delete 操作提供具体文件上下文，未跟踪文件删除确认按钮暴露 filename 上下文以及确认删除/取消保留语义，未跟踪 GGUF import editor 的 Display name 和 Family 字段暴露当前值、必填状态和 metadata 用途语义，Import/Cancel toolbar 暴露 ready/display-name-required 和 close-without-importing 语义，Storage Matrix 提供 ready/tracked/on-disk/untracked 组合 VoiceOver 摘要，Add Model 错误、模型行和详情状态消息使用可换行 message row 并提供明确辅助功能 label/value/hint。
...
- Models：模型下载、导入、删除、状态恢复入口，空状态提供无 tracked models、无 untracked GGUF files 和下载/导入入口提示，Add Model 表单支持键盘提交、可访问确认状态和 URL/display name/repository/file path/revision/family 字段级 label/value/hint，未跟踪文件行和导入 editor 字段提供文件名/大小、显示名必填、family metadata、操作上下文和删除确认语义，Storage Matrix、Add Model 错误、带模型名上下文的行内控制和详情 sheet 操作、tracked model 删除确认、Native Loading mode 与辅助模型选择器、列表状态消息和详情状态消息支持可访问 label/value/hint 与大字号可读布局。
