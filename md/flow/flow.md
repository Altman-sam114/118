...
- Models：模型下载、导入、删除、状态恢复入口，空状态提供无 tracked models、无 untracked GGUF files 和下载/导入入口提示，Add Model 表单支持键盘提交和可访问确认状态，未跟踪文件行和导入 editor 字段提供文件名/大小、显示名必填、family metadata、操作上下文和删除确认语义，Storage Matrix、Add Model 错误、带模型名上下文的行内控制和详情 sheet 操作、tracked model 删除确认、Native Loading mode 与辅助模型选择器、列表状态消息和详情状态消息支持可访问 label/value/hint 与大字号可读布局。
