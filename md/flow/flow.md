...
- Prompts：保存、分类、搜索、编辑、加载模板；Add toolbar 入口提供 Add prompt template label、Ready value，并提示会使用当前 generation parameters 打开新模板编辑器；搜索入口提供 search submit 语义和名称/分类/positive prompt/negative prompt 匹配范围提示，无模板和搜索无结果状态提供组合 VoiceOver 状态、搜索上下文和下一步提示，分类菜单提供 44pt 命中区、系统 pointer hover affordance 和分类上下文 VoiceOver 语义，模板编辑和加载控件提供可访问文本 label、模板名上下文和 44pt 命中区，模板行 metric pills 暴露 steps/sampler/canvas size 参数上下文，模板编辑器和分类重命名 editor 的 Save/Cancel toolbar 暴露 ready/name-required 和 close-without-saving 语义，模板行、模板 prompt 编辑区和共享参数编辑控件在 accessibility Dynamic Type 下保持可读，并让参数重置说明会保留 positive / negative prompt、Seed 文本框说明会编辑当前参数的 seed value、随机 seed 按钮说明当前值与生成新 seed 的操作、canvas size preset 说明会同时更新 width/height 且自定义尺寸仍可编辑、Width/Height stepper 说明会调整当前参数的画布像素尺寸、sampler picker 说明会选择当前参数的采样算法、Steps stepper 说明会调整当前参数的去噪步数、CFG slider 说明会调整当前参数的 prompt guidance strength。
...
- Prompts：提示词模板维护入口，Add prompt template toolbar 入口、空状态、搜索空状态、分类菜单 pointer hover 和分类上下文语义、带模板名上下文的模板行控制、模板行 metric pills 的 steps/sampler/canvas size 参数语义、模板 prompt 编辑区和共享参数编辑支持可访问 label、下一步提示与大字号可读布局。
