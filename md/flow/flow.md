...
- Gallery 图片删除由父级持有单一 request、phase、staging token 和错误。`AppFileStore` 先校验 PNG filename/path 边界，再把原文件 move 到 `GeneratedImages/.DeletionStaging` 的唯一 token 路径；原文件起始已缺失时 token 明确为 no-payload。随后使用新的短生命周期 `ModelContext(modelContext.container)` 按图片 UUID fetch/delete/save，不把主 context 的 model object 传入，也不对主 context delete、rollback 或 reinsert。metadata 失败时有 payload 就 restore；restore 失败保留同一 token 且只允许 Retry Restore。metadata 已不存在视为幂等终态并继续 finalize；finalize 失败保留 staged PNG，进入 logical delete + cleanup pending 且只允许 Retry Cleanup。父级错误和 token 不依赖可能被 `@Query` 移除的详情对象，只有 metadata、original 和 staged payload 全部收敛后才清理匹配的 detail path/focus 并允许详情 dismiss。现有启动 reconciliation 不处理 staging，进程中断可能留下隐藏 payload。
...
用户确认删除
  ...
  -> AppFileStore 校验 filename 和 token 路径
