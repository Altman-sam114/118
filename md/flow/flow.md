...
- Generate：输入 prompt、参数、选择模型、启动/取消生成、展示结果；Save Template toolbar 入口暴露 ready / positive prompt required 状态和保存当前 prompts/参数到 Prompt Library 的语义，prompt 编辑器 header、清除控件、参数重置作用范围、seed 文本框编辑语义、随机 seed 当前值、canvas size preset 更新 width/height 且保留自定义尺寸编辑的语义、width/height 画布像素尺寸语义、sampler 采样算法选择语义、steps 去噪步数调整语义、CFG prompt guidance strength 语义、空模型状态 Open Models 入口语义和 44pt 命中区、ready model Picker 当前选择和 ready model count 语义、运行门禁、主 Generate 按钮 ready/blocked 状态、Open Models/Edit Prompt 下一步入口、进度、取消按钮、结果预览和 Gallery handoff 提供明确可访问标签/值/提示，Gallery handoff button 会命名已保存生成图片入口并区分 saved Gallery record 与 unavailable record，并在 accessibility Dynamic Type 下减少横向挤压。
