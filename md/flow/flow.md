...
- Gallery：生成结果查看、过滤、复用入口，筛选栏、Sort menu、图块、详情预览/参数/操作、图片/folder 删除确认和 Organization 控件支持大字号可读布局、图块 prompt 垂直可读性、date/model metadata 窄宽度堆叠回退、图块 label/value 拆分语义、详情预览下一步 hint、筛选栏和图块 pointer hover affordance、当前图片与 folder 上下文、folder name 输入框必填/当前值语义、Folder 即时保存提示、草稿/已保存 tags 和清晰操作语义。
