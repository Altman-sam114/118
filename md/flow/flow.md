...
- Prompts：提示词模板维护入口，Add prompt template toolbar 入口、空状态、搜索空状态、分类标题 heading 和当前可见模板数量语义、分类菜单 pointer hover 和分类上下文语义、分类清除确认 category 名与确认/取消语义、模板行 row-level positive/negative/category/参数摘要、带模板名上下文的模板行控制、模板行 metric pills 的 steps/sampler/canvas size 参数语义和窄宽度回退、模板 name/category 与 category rename name 输入框语义、模板 prompt 编辑区和共享参数编辑支持可访问 label、下一步提示与大字号可读布局。
