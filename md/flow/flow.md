...
- Plan：当前 Local plan、付费能力规划、Capability Matrix paid candidates Planning only、entitlement rules、paid candidates Planning only availability row、Mac app Not enabled 状态和平台可用性状态入口，状态行、Capability Matrix row hints、Entitlement Rules row hints、Availability row hints、Mac readiness blocker hints、说明 note row 和 iPad panel heading/footer 提供明确辅助功能语义。
