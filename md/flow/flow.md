...
- Plan：展示当前 Local plan、StoreKit 未配置状态、能力矩阵、entitlement rules、availability rows、平台状态和 Mac readiness checklist；compact 使用单列 Form，iPad regular 使用双栏阅读布局，accessibility Dynamic Type 下回退单列；Current Build、Platform Status、Mac readiness、能力矩阵、权益规则和可用性说明使用文字、图标、颜色和描边共同表达的状态徽章，并为 VoiceOver 提供明确 label/value，避免只靠颜色或尾部压缩标签表达状态；权益规则和可用性说明明确当前本地工具保持可用、付费候选仍未售卖、Purchase UI 仍需 StoreKit 与 entitlement mapping 且未持久化 entitlement，当前 iPhone/iPad 可用，Mac Catalyst 未启用且 Mac 前置条件仍未完成。
...
- Plan：当前 Local plan、付费能力规划、entitlement rules、availability rows 和平台可用性状态入口，状态行提供明确辅助功能 label/value。
