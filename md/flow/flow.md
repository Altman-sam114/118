...
- Generate 在 compact 下保持单列表单；在 iPad regular 普通 Dynamic Type 下优先使用双栏创作台，左侧放模型/prompt/参数，右侧放状态、运行和结果，横向空间不足时回退既有单列表单；accessibility Dynamic Type 下直接回退单列，控制台 header、状态 pill 和 metrics 纵向/单列排列以保持可读，普通字号下控制台 metrics 使用 adaptive grid 在窄栏中减少横向挤压，Run 区域的 progress stage label 允许垂直展开以保留较长阶段文案，Run 区主要和次要操作按钮保持 44pt 最小命中区。
