...
- Gallery：查看、过滤、排序、复用参数、删除图片；筛选栏、空状态、图块、详情页参数/操作和 Organization 控件在 accessibility Dynamic Type 下使用可读布局、清晰辅助功能语义和足够触控区域，空状态会暴露当前筛选条件和 0 images，Sort menu 暴露当前排序值，筛选栏 rows 和图片图块提供系统 pointer hover affordance，folder rename/delete swipe 和 context menu actions 暴露 folder 名上下文，Folder/Tags/Save Tags 会暴露当前值与未保存状态。
