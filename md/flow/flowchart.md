...
读图说明：人工先提出目标，Agent A 只负责分析和写实现提示词；Agent B 才实现和测试；Agent C 验收并更新核心逻辑文档；最后交给人工复核，决定是否进入下一轮。
...
flowchart TD
  ...
  C1 --> F["更新 flow.md / flowchart.md / update_log.md"]
  F --> H2["人工复核：通过、退回或提出下一轮目标"]
  H2 --> H
```
