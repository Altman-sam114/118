...
flowchart TD
  ...
  NAV --> GALUI["Gallery：compact 内部筛选 split / iPad 可读 filter rail 和 pointer hover / folder actions 和 editor 语义 / 空状态筛选上下文语义 / Sort 当前值语义 / 图块 label/value 拆分和 pointer hover / 详情操作图片上下文语义 / Folder 即时保存和 draft/saved tags 语义"]
  ...
  SAVE --> GALLERY["Gallery 展示、可读筛选、可读图块和 pointer hover、详情 Folder 即时保存提示和草稿/已保存状态、复用参数"]
  GALLERY --> UI
```
