...
flowchart TD
  ...
  UI --> GENUI["Generate：compact 单列表单 / iPad 双栏创作台 / Save Template readiness 语义 / 可读 prompt editor / 可读参数控件 / 参数重置语义 / Seed 输入语义 / 随机 seed 语义 / 尺寸 preset 和宽高像素语义 / sampler 算法语义 / steps 去噪语义 / CFG guidance 语义 / 空模型入口语义 / 运行状态和运行入口下一步语义"]
