...
flowchart TD
  ...
  UI --> PROMPTUI["Prompts：模板分类 / 搜索入口 submit 和匹配范围语义 / 分类菜单 pointer hover 和上下文语义 / 空状态语义 / 模板 metric steps/sampler/size 语义 / 可读编辑器 / 带模板名上下文的 controls / 共享参数重置、Seed 输入、随机 seed、尺寸 preset、宽高、sampler、steps 和 CFG 语义"]
