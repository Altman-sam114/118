...
flowchart TD
  ...
  UI --> MODELUI["Models：下载 / 导入 / 删除 / Add Model 键盘提交 / Storage Matrix VoiceOver 摘要 / Add Model error row / 可访问 controls / 列表和详情 message rows"]
  ...
  NAV --> PLANUI["Plan：compact Form / iPad 双栏 / 可访问状态徽章和 note rows / 能力矩阵 / entitlement rules / availability / Mac readiness"]
  ...
  SAVE --> GALLERY["Gallery 展示、可读筛选、可读图块和详情、复用参数"]
  GALLERY --> UI
```
...
```mermaid
flowchart TD
  A["用户打开 Models：Add Model 键盘提交 / Storage Matrix 摘要 / 可访问控件 / 大字号堆叠"] --> B{"模型来源"}
