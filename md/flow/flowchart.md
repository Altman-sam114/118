...
flowchart TD
  ...
  UI --> NAV["Root 导航：iPhone TabView / iPad 单层 SplitView / neutral Plan navigation icon / 可访问 sidebar rows / pointer hover affordance"]
  ...
  NAV --> PLANUI["Plan：compact Form / iPad 双栏 / neutral planning overview icon / summary row hints / panel heading 和 note 语义 / 可访问状态徽章和 note rows / 能力矩阵 Paid candidates Planning only 和 row hints / entitlement rule hints / availability hints 含 Paid candidates Planning only 与 Mac app Not enabled / Mac readiness blocker hints"]
