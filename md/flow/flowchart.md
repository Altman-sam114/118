...
flowchart TD
  ...
  SAVE --> GALLERY["Gallery 展示、可读筛选、可读图块和 pointer hover、folder create/rename 名称冲突 gate 且历史记录不迁移、folder 和图片删除确认上下文、Folder 即时保存、Save Tags normalized dirty-state gate 和 canonical 回显、复用参数"]
  GALLERY --> UI
```
