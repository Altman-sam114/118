...
flowchart TD
  ...
  UI --> GENUI["Generate：compact 单列表单 / iPad 双栏创作台优先且窄 regular 宽度回退单列 / Save Template readiness 语义 / 可读 prompt editor 和字段化字符计数语义 / 可读参数控件 / 参数重置语义 / Seed 输入语义 / 随机 seed 语义 / 尺寸 preset 和宽高像素语义 / sampler 算法语义 / steps 去噪语义 / CFG guidance 语义 / 空模型状态卡片摘要和入口语义 / 模型选择语义 / 控制台 backend/model readiness 汇总语义 / 运行状态和运行入口下一步语义 / 结果 Gallery handoff saved/unavailable 语义"]
