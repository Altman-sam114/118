...
flowchart TD
  ...
  UI --> MODELUI["Models：下载 / 导入 / 删除 / toolbar refresh/add menu 语义 / 空状态 no-model/no-untracked 语义 / Add Model 键盘提交 / 未跟踪文件操作语义 / 未跟踪 GGUF import editor 语义 / Storage Matrix VoiceOver 摘要 / Add Model error row / 带模型名上下文的 row controls 和详情操作按钮 / 列表和详情 message rows"]
...
```mermaid
flowchart TD
  A["用户打开 Models：空状态下载/导入入口语义 / Add Model 键盘提交 / 未跟踪文件语义 / Storage Matrix 摘要 / 带模型名上下文的控件 / 大字号堆叠"] --> B{"模型来源"}
