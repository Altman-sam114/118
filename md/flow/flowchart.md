...
flowchart TD
  ...
  NAV --> GALUI["Gallery：compact 内部筛选 split / iPad 可读 filter rail 和 pointer hover / folder actions、name field 和 delete confirmation 语义 / 空状态筛选上下文语义 / Sort 当前值语义 / 图块 date/model metadata 窄宽度回退、label/value 拆分和 pointer hover / 详情预览下一步 hint / 详情操作和删除确认图片上下文语义 / Folder 即时保存和 draft/saved tags 语义"]
