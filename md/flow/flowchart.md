...
flowchart TD
  ...
  UI --> PROMPTUI["Prompts：模板分类 / 空状态语义 / 可读编辑器 / 可访问 controls / 共享参数编辑"]
  UI --> NAV["Root 导航：iPhone TabView / iPad 单层 SplitView / 可访问 sidebar rows"]
  NAV --> GALUI["Gallery：compact 内部筛选 split / iPad 可读 filter rail / 可读图块、详情和组织控件"]
  NAV --> PLANUI["Plan：compact Form / iPad 双栏 / panel heading 和 note 语义 / 可访问状态徽章和 note rows / 能力矩阵 / entitlement rules / availability 含 Paid candidates Planning only 与 Mac app Not enabled / Mac readiness"]
  ...
  SAVE --> GALLERY["Gallery 展示、可读筛选、可读图块、详情和组织控件、复用参数"]
  GALLERY --> UI
```
