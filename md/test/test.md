...
若本轮改动 native Release asset 元数据，还要运行：
...
- 下载后必须先按 `NativeBackend/StableDiffusionCpp/native-backend-asset.json` 校验 SHA-256，再解压 XCFramework。
- 校验日志写入 `ci-results/native-backend-asset.log`，manifest 记录期望摘要、实际摘要和校验结果。
- 若 SHA-256 不一致，workflow 不运行 native preflight 和 xcodebuild，而是把失败原因写入结果包。
...
- `junit.xml` 或等价摘要。
- `xcodebuild.log`、`ci-failure-summary.md`、native asset 校验、native preflight 和 Swift parse 日志。

若当前仓库没有 `origin`，或没有权限访问 GitHub Actions，必须记录云端验证阻塞；不得伪造 push、run id 或 artifact 核对。

## Agent X 循环验证规则

Agent X 只负责主控调度，不改变每轮验证门槛。每个 Agent X 小轮次仍必须按以下顺序闭环：

```text
Agent A 写版本化提示词
  -> Agent B 本地轻量检查
  -> Agent B 版本 commit 并 push origin/main
  -> GitHub Actions 生成未加密 artifact
  -> Agent C 下载最新结果包并复判
  -> Agent X 根据 Agent C 结论继续、退回、暂停或完成
```

Agent X 验证规则：

- 不得跳过 Agent C artifact 验收。
- 不得用旧 run、旧 artifact、本地输出或 Agent B 文字汇报冒充最新云端结果。
- Agent C 未通过时，Agent X 不能继续下一轮并伪装成功；只能退回 Agent B 修复或暂停等待人工。
- 连续失败、连续无有效 diff、权限缺失、账号问题、密钥或付费服务需求必须触发暂停或停止判断。
- 每轮最终仍记录具体命令、退出结果、commit SHA、run id、run attempt 和 artifact 名称。
...
- `Scripts/check-video-model-package.sh` 使用临时小型 `@main` Swift fixture probe（只写入 `/private/tmp`）编译 `VideoModelPackageService.swift` 与 `AppFileStore.swift`，调用真实 `VideoModelPackageInspector` 和 `AppFileStore`，覆盖有效 `.ldvideo` 导入、重复目录名、未知扩展名、空文件、缺文件和 `../` 路径边界；不下载模型、不创建 SwiftData 数据。
- 本地实际命令：`./Scripts/check-video-model-package.sh`，本轮修复前后均需退出码 `0`。
- `.github/workflows/ci-results.yml` 在 `Run validation commands` 中执行该脚本，把输出写入 `ci-results/video-model-package.log`，并将其作为静态检查纳入 manifest/JUnit/failure summary；脚本失败会使 workflow 失败。
