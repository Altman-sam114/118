...
private struct PlanView: View {
    ...
    private let availabilityItems = [
        ...
        PlanAvailabilityItem(
            title: "Purchase UI",
            detail: "Purchase UI remains hidden; this build shows no purchase sheet, buy button, unlock entry point, restore entry point, manage subscription entry point, or product loading state until StoreKit products, entitlement mapping, and restore/receipt decisions exist.",
            status: PlanStatusToken(title: "Requires configuration", systemImage: "wrench.and.screwdriver", color: SciFiTheme.amber),
            systemImage: "cart",
            accessibilityHint: "Clarifies there is no purchase sheet, buy button, unlock entry point, restore entry point, manage subscription entry point, or product loading state in this build."
        ),
        PlanAvailabilityItem(
            title: "Mac app",
