...
- Prompt library with categories, category action menus with pointer hover affordance and category-aware VoiceOver context, accessible labeled template controls with template-name context, category rename/clear actions, VoiceOver-aware empty and search-empty states, Dynamic Type-friendly prompt rows, editable saved templates with labeled prompt editors, direct saving from the generation screen, and one-click loading into the generation screen.
- Plan screen that truthfully shows the current Local plan, states that StoreKit products are not configured, presents platform status, a capability matrix with paid candidates marked planning-only, entitlement rules, availability rows including paid candidates as planning-only and Mac app not-enabled status, readable note rows, and Mac readiness blockers with Dynamic Type-friendly status badges, explicit VoiceOver semantics, and iPad panel heading/note landmarks, and uses an iPad regular two-column layout without enabling purchases, entitlements, or Mac Catalyst.
