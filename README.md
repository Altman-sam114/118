...
- Plan screen that truthfully shows the current Local plan, uses a neutral planning/checklist overview icon instead of a payment-coded icon, states that StoreKit products are not configured, presents platform status with summary row VoiceOver hints, a capability matrix with paid candidates marked planning-only and row-level VoiceOver hints, entitlement rules with row-level VoiceOver hints, availability rows including paid candidates as planning-only and Mac app not-enabled status with VoiceOver hints plus Availability footer context, readable note rows, and Mac readiness blockers with Dynamic Type-friendly status badges including a `Needs QA` window/sidebar validation status plus platform/native/UI/signing VoiceOver hints, explicit VoiceOver semantics, compact section footer note context, and iPad panel heading/note landmarks, and uses an iPad regular two-column layout without enabling purchases, entitlements, or Mac Catalyst.
