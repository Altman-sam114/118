...
- Verified locally: The production `AppFileStore` real-filesystem probe passed normal bytes-preserving stage/restore/finalize, idempotency, sourceMissing, inside/outside staging symlinks, real same-volume resource values, injected missing/mismatched volume identity, and all four move-error file states. Fixed repository checks, both 12-file Swift parse commands, and an additional 12-file iOS typecheck also exited 0; GitHub Actions remains the full build authority for Agent C.
