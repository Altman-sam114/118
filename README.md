...
- Plan screen that truthfully shows the current Local plan, states that StoreKit products are not configured, presents platform status, a capability matrix with paid candidates marked planning-only and row-level VoiceOver hints, entitlement rules with row-level VoiceOver hints, availability rows including paid candidates as planning-only and Mac app not-enabled status with VoiceOver hints, readable note rows, and Mac readiness blockers with Dynamic Type-friendly status badges plus platform/native/UI/signing VoiceOver hints, explicit VoiceOver semantics, and iPad panel heading/note landmarks, and uses an iPad regular two-column layout without enabling purchases, entitlements, or Mac Catalyst.
