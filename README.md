...
- SwiftUI app shell with adaptive tab navigation for iPhone and a single top-level split-view layout for iPad, including iPadOS 17+ hardware-keyboard Command-1...5 navigation for Generate, Models, Gallery, Prompts, and Plan through the same Root selection used by compact tabs and the regular-width sidebar. The named Navigate commands remain disabled when startup fails before Root provides its focused selection binding. Accessible sidebar rows include pointer hover affordance, the embedded Gallery filter rail avoids nested split views and can fall back to a top filter shelf in constrained regular-width windows, and the neutral planning/checklist Plan entry has a sidebar hint covering local plan, planning-only paid capability status, and platform readiness. Mac Catalyst remains disabled.
...
- Independent local video model deployment records and `VideoModels` storage for explicit `.ldvideo` directory packages. Import validates `manifest.json`, the declared `video-diffusion` type and `video-generation` capability, required non-empty files, path boundaries, declared sizes, and SHA-256 integrity before the package is marked Deployed. The Models UI keeps deployment, inference availability, package errors, and import progress distinct and readable in compact, regular, and accessibility text-size layouts, with accessible package actions and removal confirmation. Deployed packages remain separate from image `LocalModel` records and Generate; this build reports `Inference unavailable` and does not generate video.
- Video inference has an independent `VideoGenerationRequest` / `VideoGenerationBackend` contract and an `UnavailableVideoGenerationBackend`. `NativeBackend/StableDiffusionCpp/video-native-dependency-manifest.json` and `Scripts/check-video-native-dependency.sh` record engine revision, public ABI, model compatibility, license, and provenance evidence without guessing missing facts. Native symbols are reported as observed-only and never treated as ABI support; the gate remains dependency-blocked and video inference remains unavailable.
- Hugging Face GGUF download flow with paste-and-parse Hugging Face file URLs, keyboard-aware Add Model submission, Add Model field-level VoiceOver labels/values/hints for URL, display name, repository, GGUF file path or direct URL, revision, and family, `.gguf` source validation, toolbar refresh/add menu VoiceOver context, local GGUF file import, progress, pause, resume, cancel, confirmed deletion with model-name/count VoiceOver context, duplicate protection, persisted byte tracking, untracked-file import/cleanup with file-specific VoiceOver actions and filename-aware delete confirmation semantics, an untracked GGUF import editor with display-name and family field VoiceOver semantics plus Import/Cancel ready and display-name-required toolbar semantics, restart recovery for interrupted downloads, accessible labeled row controls and detail sheet action buttons with model-name context, detail Native Loading mode and auxiliary model picker current-value VoiceOver context, an all-empty-state summary that names no tracked GGUF models, no untracked GGUF files, and no deployed video packages while offering Download from Hugging Face, Import GGUF File, and Import Video Package, readable Add Model errors, readable model list and detail status messages, and Dynamic Type-friendly model storage rows with a combined VoiceOver storage summary, a Storage Matrix header that falls back from horizontal title/ready layout to stacked layout in constrained ordinary Dynamic Type widths, and Storage Matrix metrics that prefer the existing two-column grid at ordinary text sizes, fall back to stacked title/value metrics when constrained, and stack directly at accessibility text sizes, and Models list row headers and footers that fall back from horizontal identity/status or size/controls layout to stacked layout in constrained ordinary Dynamic Type widths, and untracked model file rows that fall back from horizontal file/actions layout to stacked layout when constrained. Video package deployment/checking remains separate and this build reports `Inference unavailable`.
- Generation screen with positive and negative prompts, Dynamic Type-friendly prompt editor headers with field-specific character-count VoiceOver semantics, clear controls, and a constrained-width title/metadata header fallback and a constrained-width character-count/clear metadata fallback, Save Template toolbar readiness and prompt-required VoiceOver semantics, shared parameter controls with reset-defaults scope, prompt-preservation semantics, seed field editing hints, random seed current-value hints, narrow-width seed row fallback, canvas size preset hints for width/height updates plus editable custom dimensions, narrow-width size preset fallback, steps/width/height Stepper label width fallback, width/height hints for canvas pixel dimensions, sampler picker hints for choosing the sampling algorithm, steps hints for denoising-step adjustment, CFG hints for prompt guidance strength plus narrow-width header fallback, VoiceOver-aware generation readiness, no-model empty-state summary semantics plus Open Models next-step semantics with a 44pt hit area, model picker current-selection and ready-model-count semantics, ready/blocked run action and next-step semantics with 44pt minimum hit areas for Generate, Cancel, Open Models, and Edit Prompt, adaptive console metrics with constrained-width SciFiMetric icon/copy fallback, readable progress stage text, progress/cancel/result semantics, explicit cancellation state, result display, a Gallery handoff with a 44pt minimum hit area that names the saved result entry and distinguishes saved versus unavailable Gallery records, and an iPad two-column creation console that separates inputs from run/result status while summarizing backend/model readiness for VoiceOver, falling back to a single readable column at accessibility Dynamic Type sizes or constrained regular-width windows. The console overview keeps its constrained-width horizontal-to-stacked fallback while deriving one icon/color/title/detail/combined-VoiceOver state from backend readiness and the selected ready model: backend offline, backend ready with no selected ready model, debug mock with a selected model, or native with a selected model. Offline and no-model states take priority, mock output is identified as a UI development placeholder, selected models are described as ready rather than preloaded, and overview readiness does not replace the separate positive-prompt generation gate. Console status pills independently retain their constrained-width horizontal-to-stacked fallback.
- Gallery grid with a Dynamic Type-friendly filter rail that can fall back to a top filter shelf when the iPad embedded regular-width layout is constrained, readable folder/tag filters with image counts and pointer hover affordance, folder rename/delete actions with folder-name VoiceOver context, folder delete confirmation button VoiceOver context for confirming or cancelling folder deletion, and a shared New/Rename folder-name validator that trims names, uses localized case-insensitive conflict comparison, shows required/conflict/available inline states, and keeps Save, its action guard, and VoiceOver synchronized. Exact trimmed original names remain valid during rename, while other changes exclude the current folder UUID before conflict checks; existing duplicate records are not migrated and remain separate by UUID. The Gallery also provides empty-state VoiceOver context for the active filter, Dynamic Type-friendly image tiles with pointer hover affordance, prompt text that can preserve its existing line budget vertically in constrained tile widths, readable metadata that can fall back from horizontal date/model display to stacked model names in constrained tile widths, prompt-focused tile accessibility labels with model/date/output metadata values, readable detail preview next-step hints, detail parameter rows that fall back from horizontal title/value to stacked layout in constrained ordinary Dynamic Type widths, readable detail parameters/actions/organization controls with image-context VoiceOver values, image delete confirmation button VoiceOver context for confirming or cancelling deletion, and Folder picker VoiceOver context for immediate selection saving. If that save fails, the previous optional folder assignment is restored and an operation-specific native alert explains how to retry. Save Tags is enabled only when normalized draft tags differ from saved tags, exposes matching VoiceOver state, restores canonical saved CSV plus the disabled state after a successful save, and on failure restores the previous saved tags while preserving the exact text draft and enabled retry state. Folder and tag failures share one identifiable detail-local alert state, without rolling back the wider SwiftData context. Image deletion moves the PNG into a hidden same-volume staging directory before an isolated short-lived SwiftData context fetches and deletes metadata by UUID. Staging rejects symbolic links, resolved path escape, unavailable volume identity, and cross-volume moves before metadata changes; its opaque store-issued token cannot be rebuilt from URL fields. Volume validation uses every actual existing original/staged payload URL and compares both actual files when both exist; only a missing endpoint falls back to its already-validated images/staging directory, so sourceMissing never requests resource values from a nonexistent file. A move error is classified from the real original/staged four-state result, and a disappeared original never falls back to ordinary failure or sourceMissing. If both original and staged files exist, Gallery retains the same token in a blocking Restore Pending state instead of allowing another Delete; Retry Restore never overwrites either file, stays blocked while both exist, and safely converges after external state becomes original-only or staged-only. Metadata failure restores the staged PNG; restore failure retains the token and offers only Retry Restore. While that exact UUID/filename request remains restore-pending, on-appear and Refresh reconciliation preserve its metadata while continuing to reconcile every other image. Metadata that is already absent continues to cleanup, while cleanup failure is reported as logical deletion with staged PNG cleanup pending and offers only Retry Cleanup. Retry Metadata, Retry Restore, and Retry Cleanup expose current-image VoiceOver context. Navigation and detail dismissal happen only after full cleanup succeeds. The staging token is session-local, so an app termination between stages can leave a hidden payload for later maintenance; cross-launch staging reconciliation remains intentionally out of scope. Editable folders, folder and saved-tag filtering, date/model sorting with current sort VoiceOver value, PNG sharing, missing-file and orphan-file reconciliation, parameter reuse, one-tap regeneration, and separate requested/output image dimensions remain available.
- Prompt library with categories, category headings that expose the current visible template count after filtering and fall back from horizontal title/actions layout to stacked layout in constrained ordinary Dynamic Type widths, an Add prompt template toolbar action that exposes ready state and opens a new template editor using the current generation parameters, a search field with search-submit semantics and VoiceOver context for template name/category/prompt matching, category action menus with pointer hover affordance and category-aware VoiceOver context, category clear confirmation buttons with category-name confirm/cancel VoiceOver semantics, row-level template summaries that expose positive prompt, negative prompt, category, steps, sampler, and canvas size while preserving Edit/Load controls, and a discoverable trash Delete control whose explicit and swipe entry points share one template-name confirmation and stable-ID single-target deletion path, with Cancel or dismissal preserving the template. Edit, Load, and Delete keep 44pt targets and pointer, keyboard, and template-aware VoiceOver semantics; they stay horizontal when ordinary Dynamic Type width permits, fall back to a vertical group when constrained, and stack directly at accessibility sizes. Template metric pills expose steps, sampler, and canvas size context to VoiceOver and fall back from horizontal to stacked layout in constrained ordinary Dynamic Type widths, while category rename/clear actions, VoiceOver-aware empty and search-empty states, Dynamic Type-friendly prompt rows, editable saved templates with labeled prompt editors, template name/category field VoiceOver semantics, field-specific prompt character-count VoiceOver semantics with a constrained-width title/count header fallback, shared parameter controls with narrow-width seed, size preset, Stepper labels, and CFG header fallback, category rename name-field semantics, Save/Cancel ready and name-required VoiceOver semantics, direct saving from the generation screen, and one-click loading into the generation screen remain available.
- Plan screen that truthfully shows the current Local plan, uses a neutral planning/checklist overview icon instead of a payment-coded icon, with a constrained-width overview fallback from horizontal icon/copy layout to stacked layout at ordinary Dynamic Type sizes, states that StoreKit products are not configured, uses the overview note/value/hint and Current Build purchase note to clarify that paid candidates on the screen are planning only and cannot be bought or unlocked here, presents platform status with summary row VoiceOver hints, width-aware summary rows that prefer horizontal title/status pairs at ordinary Dynamic Type and fall back to stacked rows when constrained, and a Mac support note that keeps Mac/Catalyst support planned and not enabled while the iPhone/iPad app remains available and says there is no separate Mac binary, Mac signing profile, sandbox entitlement, or notarization pipeline, a capability matrix with paid candidates marked planning-only and StoreKit purchases marked not enabled/not selling, purchasing, or unlocking paid candidates, entitlement rules that mark Generate, Models, Gallery, and Prompts as always available and not gated by purchases in this build, with row-level VoiceOver hints including paid candidates that grant no trial entitlement, preview entitlement, feature flag, or unlock gate, entitlement persistence that has no local cache, cross-launch restoration, server-side entitlement source, or receipt-backed state, plus a StoreKit purchase gate that does not request an App Store product and has no restore button, receipt validation path, or entitlement mapping resolution until StoreKit prerequisites are configured, availability rows including paid candidates as planning-only, Purchase UI hidden with no purchase sheet, buy button, unlock entry point, restore entry point, manage subscription entry point, or product loading state until StoreKit products, entitlement mapping, and restore/receipt decisions exist, and Mac app status that says the iPhone/iPad app is available while Mac/Catalyst is not enabled with no separate Mac binary, Catalyst entitlement set, or desktop distribution channel plus Availability footer context, readable note rows that keep horizontal icon/text notes at ordinary Dynamic Type and stack the icon above the note text at accessibility text sizes, and Mac readiness blockers with footer context that separates the available iPhone/iPad app from the not-enabled Mac/Catalyst app, Dynamic Type-friendly status badges and status rows that prefer horizontal title/status headers at ordinary Dynamic Type and fall back to stacked rows when constrained or using accessibility text sizes, including Apple platform support that states this build is still iPhone/iPad app target only, native backend slice status that states the native inference framework still needs a Mac or Catalyst slice, `Needs QA` window/sidebar QA validation that says current iPad layout and pointer affordance do not replace dedicated Mac/Catalyst window/sidebar/keyboard/pointer validation, and signing/distribution status that states Mac release channel, signing, sandboxing, and notarization still need a product decision while no Mac signing profile, sandbox entitlement, or notarization pipeline exists, explicit VoiceOver semantics, compact section footer note context, and iPad panel heading/note landmarks, and uses an iPad regular two-column layout without enabling purchases, entitlements, or Mac Catalyst.
- Shared Sci-Fi status pills that keep icon/title horizontal when they fit, fall back to a stacked icon/title layout in constrained ordinary Dynamic Type widths, and use the stacked multi-line layout at accessibility text sizes while preserving color, capsule treatment, and combined accessibility semantics.
...
- Completed: Hardened Gallery deletion recovery boundaries with a store-issued opaque token, resolved staging containment and symlink rejection, fail-closed same-volume identity checks, production move-error four-state classification, exact restore-pending reconciliation protection, and image-context VoiceOver values for retry actions.
- Verified locally: The production `AppFileStore` real-filesystem probes passed normal bytes-preserving stage/restore/finalize, idempotency, sourceMissing, inside/outside staging symlinks, real same-volume resource values, injected missing/mismatched directory and actual-payload volume identities across stage/restore/finalize, and all four move-error file states. Fixed repository checks, both 12-file Swift parse commands, an additional 12-file iOS typecheck, and the follow-up conflict-catch ordering/token-retention contract check also exited 0; GitHub Actions remains the full build authority for Agent C.
- Risk: Simulator, manual iPhone/iPad Refresh and VoiceOver, SwiftData save-failure UI, rapid interaction, process termination, and a physical second-volume move were not run. Cross-volume refusal used the production comparison with injected identities; cross-launch staged payload recovery remains unimplemented.

### 2026-07-26

- Completed: Made Gallery image deletion recoverable with hidden same-volume PNG staging, isolated UUID-based SwiftData deletion, compensating restore, honest cleanup-pending state, token-scoped retries, and success-only detail/navigation cleanup.
- Verified: Local lightweight checks, both fixed 12-file Swift parse commands, a full 12-file iOS typecheck, a production `AppFileStore` real-filesystem probe, and an in-memory SwiftData isolated-context probe are required for v1.159; the pushed GitHub Actions run and artifact remain for Agent C review.
- Risk: Manual iPhone/iPad, VoiceOver, rapid interaction, controlled SwiftData save-failure UI, and process-termination recovery were not run. Staging tokens are current-session only, so a termination during deletion can leave a hidden staged PNG.
- Completed: Made the Generate console overview derive one truthful visual and combined VoiceOver state from backend readiness and the selected ready model, with priority ordered as backend offline, ready backend without a selected ready model, debug mock plus model, then native plus model. The mock-title check is centralized and reused by the unchanged ready branch of the generation gate; overview readiness remains separate from positive-prompt readiness.
- Verified: Local lightweight checks, both fixed 12-file Swift parse commands, and a source-backed four-state mapping probe are required for v1.158; the pushed GitHub Actions run and artifact remain for Agent C review.
- Risk: Manual iPhone/iPad Dynamic Type and VoiceOver checks are deferred; static mapping and parse checks do not validate runtime layout, spoken order, or every backend configuration.
- Completed: Added named iPadOS 17+ Navigate menu commands for Command-1...5, mapping in order to Generate, Models, Gallery, Prompts, and Plan. Scene commands use a focused binding to the existing Root selection shared by compact tabs and the regular sidebar; startup failure exposes no binding, so all five commands are disabled. Mac Catalyst remains disabled.
- Verified: Local lightweight checks, both fixed 12-file Swift parse commands, an iOS 17 focused-command typecheck probe, and a source-backed production mapping probe are required for v1.157; the pushed GitHub Actions run and artifact remain for Agent C review.
- Risk: Physical iPad hardware-keyboard behavior, system shortcut discoverability, text-input focus, compact/regular transitions, and controllable startup-failure interaction are deferred to manual validation; static probes and cloud build do not replace those checks.
- Completed: Replaced the Gallery detail Folder and Save Tags silent saves with explicit error handling. Folder failures restore the previous optional folder assignment; tag failures restore the previous saved tags while retaining the exact user draft and normalized dirty-state retry behavior. Both operations use one identifiable native alert state, and successful tag saves still canonicalize the visible CSV.
- Verified: Local lightweight checks and both fixed 12-file Swift parse commands are required for v1.156; the pushed GitHub Actions run and artifact remain for Agent C review.
- Risk: Save-failure injection, local simulator interaction, real VoiceOver, iPad, and Dynamic Type checks are deferred. The implementation intentionally restores only the field changed by the failed detail operation and does not roll back the wider SwiftData context.
- Completed: Added one pure derived required/conflict/valid state for Gallery New Folder and Rename Folder. It trims both candidate and existing names, compares with `localizedCaseInsensitiveCompare`, permits the exact trimmed original name during rename, excludes the current UUID for other rename checks, and synchronizes inline status, Save gating/action guard, and VoiceOver without migrating existing duplicate folders.
- Verified: Local lightweight checks and both fixed 12-file Swift parse commands are required for v1.155; the pushed GitHub Actions run and artifact remain for Agent C review.
- Risk: Existing duplicate folders remain separate by UUID by design. Local iPad, keyboard, Dynamic Type, real VoiceOver, and historical-duplicate interaction checks are deferred, and the existing silent `try?` save-failure behavior remains unchanged.
- Completed: Connected Gallery Save Tags to the existing normalized dirty state, canonicalized the visible CSV after explicit save, and keyed detail draft identity to the selected image so drafts do not cross image IDs.
- Verified: Local lightweight checks and both fixed 12-file Swift parse commands are required for v1.154; the pushed GitHub Actions run and artifact remain for Agent C review.
- Risk: This is Gallery tag save-state behavior only; local simulator, VoiceOver, pointer, Full Keyboard Access, Dynamic Type, and image-switch interaction checks are deferred, and the existing silent `try?` save-failure behavior remains unchanged.
- Completed: Added a visible Prompt template Delete action and routed both explicit and swipe deletion through one template-name confirmation that deletes only the stable UUID target after destructive confirmation; Cancel and dismissal preserve the template. The Edit/Load/Delete group keeps 44pt controls, pointer/keyboard/VoiceOver semantics, ordinary-width horizontal layout with narrow fallback, and direct accessibility-size stacking.
- Verified: Local lightweight checks and both fixed 12-file Swift parse commands are required for v1.153; the pushed GitHub Actions run and artifact remain for Agent C review.
- Risk: This is Prompt template deletion UI and confirmation flow only; no schema, Edit/Load, search, category, editor, generation, native backend, workflow behavior, or local pointer/keyboard/VoiceOver/simulator validation is changed.
- Completed: Added a constrained-width fallback to Storage Matrix metrics so ordinary Dynamic Type prefers the existing two-column grid and falls back to stacked title/value metrics, while accessibility text sizes use stacked metrics directly.
- Verified: Local lightweight checks and both 12-file Swift parse commands are required for v1.152; the pushed GitHub Actions run and artifact remain for Agent C review.
- Risk: This is Storage Matrix metric layout only; no metric order, copy, color, formatter, panel, accessibility summary, header, ready pill, model logic, native backend, workflow behavior, or local simulator validation is changed.
- Completed: Clarified that Generate, Models, Gallery, and Prompts are always available and are not gated by purchases in this build, using an `Always available` status with a checkmark while preserving the Core local tools row icon.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.151 version.
- Risk: This is Plan entitlement-rule wording and status semantics only; no StoreKit capability, purchase gate, entitlement persistence, business behavior, Mac Catalyst, native backend, SwiftData schema, Xcode project, workflow behavior, or real device visual validation is changed.
- Completed: Added a constrained-width fallback to shared SciFiStatusPill views so icon/title stay horizontal when they fit, stack in narrow ordinary Dynamic Type widths, and use a multi-line stacked layout at accessibility text sizes.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.150 version.
- Risk: This is shared status pill layout only; no caller state, generation gate, model or prompt data, SwiftData schema, file storage, native backend, StoreKit, Mac Catalyst, Xcode project, workflow behavior, or real iPad/Mac window screenshot validation is changed.

### 2026-07-12

- Completed: Added a constrained-width fallback to Gallery detail parameter rows so title/value stay horizontal when they fit and stack in narrow ordinary Dynamic Type detail panes, and synced the Plan overview narrow-width description in flow.md.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.149 version.
- Risk: This is Gallery detail parameter layout plus a flow.md Plan overview doc sync only; no filter/sort/tile navigation, delete, folder/tags, Generate handoff, SwiftData schema, file storage, native backend, StoreKit, Mac Catalyst, Xcode project, workflow behavior, or real iPad Split View screenshot validation is changed.
- Completed: Added a constrained-width fallback to the Plan overview icon/copy header so ordinary Dynamic Type can keep a horizontal pair and fall back to stacked layout in narrow widths.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.148 version.
- Risk: This is Plan overview layout only; no StoreKit products, purchase UI, restore flow, receipts, entitlement persistence, paid gate, Mac Catalyst, Xcode project platform settings, native backend, SwiftData schema, workflow behavior, or real iPad/Mac window screenshot validation is changed.
- Completed: Added a constrained-width fallback to shared SciFiMetric cards so icon and title/value stay horizontal when they fit and stack in narrow ordinary Dynamic Type cells.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.147 version.
- Risk: This is shared SciFiMetric layout only; no console metric values, generation gate, ParameterEditor controls, run/result behavior, SwiftData schema, file storage, native backend, StoreKit, Mac Catalyst, Xcode project, workflow behavior, or real iPad Split View screenshot validation is changed.
- Completed: Added a constrained-width fallback to Generate console status pills so backend and model readiness pills stay horizontal when they fit and stack in narrow ordinary Dynamic Type rows.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.146 version.
- Risk: This is Generate console status pills layout only; no console overview copy, generation gate, ParameterEditor, run/result behavior, SwiftData schema, file storage, native backend, StoreKit, Mac Catalyst, Xcode project, workflow behavior, or real iPad Split View screenshot validation is changed.
- Completed: Added a constrained-width fallback to the Generate Local Render Console overview so icon and copy stay horizontal when they fit and stack in narrow ordinary Dynamic Type rows.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.145 version.
- Risk: This is Generate console overview layout only; no console status pills, generation gate, ParameterEditor, run/result behavior, SwiftData schema, file storage, native backend, StoreKit, Mac Catalyst, Xcode project, workflow behavior, or real iPad Split View screenshot validation is changed.
- Completed: Added a constrained-width fallback to Generate prompt editor metadata so character count and clear controls stay horizontal when they fit and stack in narrow ordinary Dynamic Type rows, and synced the flowchart Generate node summary for the recent prompt header/metadata fallbacks.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.144 version.
- Risk: This is Generate prompt metadata layout plus flowchart summary only; no generation gate, ParameterEditor, run/result behavior, SwiftData schema, file storage, native backend, StoreKit, Mac Catalyst, Xcode project, workflow behavior, or real iPad Split View screenshot validation is changed.
- Completed: Added a constrained-width fallback to Generate prompt editor headers so title and character-count/clear metadata stay horizontal when they fit and stack in narrow ordinary Dynamic Type rows.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.143 version.
- Risk: This is Generate prompt editor header layout only; no generation gate, ParameterEditor, run/result behavior, SwiftData schema, file storage, native backend, StoreKit, Mac Catalyst, Xcode project, workflow behavior, or real iPad Split View screenshot validation is changed.
- Completed: Added a constrained-width fallback to Prompt template editor prompt headers so title and character count stay horizontal when they fit and stack in narrow ordinary Dynamic Type rows.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.142 version.
- Risk: This is Prompt template editor header layout only; no template CRUD, Generate prompt editor, ParameterEditor, SwiftData schema, file storage, native backend, StoreKit, Mac Catalyst, Xcode project, workflow behavior, or real iPad Split View screenshot validation is changed.
- Completed: Added a constrained-width fallback to Prompt Library category headers so title and category actions stay horizontal when they fit and stack in narrow ordinary Dynamic Type rows.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.141 version.
- Risk: This is Prompt category header layout only; no rename/clear business logic, template row behavior, editor, search, SwiftData schema, file storage, native backend, StoreKit, Mac Catalyst, Xcode project, workflow behavior, or real iPad Split View screenshot validation is changed.
- Completed: Added a constrained-width fallback to Prompt Library template row headers so template copy and Edit/Load actions stay horizontal when they fit and stack in narrow ordinary Dynamic Type rows.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.140 version.
- Risk: This is Prompt template row header layout only; no template CRUD, load behavior, metric pill semantics, category actions, SwiftData schema, file storage, native backend, StoreKit, Mac Catalyst, Xcode project, workflow behavior, or real iPad Split View screenshot validation is changed.
- Completed: Added a constrained-width fallback to the Models Storage Matrix header so title and ready status stay horizontal when they fit and stack in narrow ordinary Dynamic Type rows.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.139 version.
- Risk: This is Storage Matrix header layout only; no tracked/untracked row business logic, download manager, detail sheet, SwiftData schema, file storage, native backend, StoreKit, Mac Catalyst, Xcode project, workflow behavior, or real iPad Split View screenshot validation is changed.
- Completed: Added a constrained-width fallback to Models untracked file rows so filename/size and Import/Delete actions stay horizontal when they fit and stack in narrow ordinary Dynamic Type rows.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.138 version.
- Risk: This is untracked model row layout only; no import/delete business logic, tracked ModelRow behavior, download manager, detail sheet, SwiftData schema, file storage, native backend, StoreKit, Mac Catalyst, Xcode project, workflow behavior, or real iPad Split View screenshot validation is changed.
- Completed: Added a constrained-width fallback to Models list row footers so size labels and row controls stay horizontal when they fit and stack in narrow ordinary Dynamic Type rows.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.137 version.
- Risk: This is Models row footer layout only; no download/import/delete behavior, detail sheet, untracked import, SwiftData schema, file storage, native backend, StoreKit, Mac Catalyst, Xcode project, workflow behavior, or real iPad Split View screenshot validation is changed.
- Completed: Added a constrained-width fallback to Models list row headers so model identity and status pills stay horizontal when they fit and stack in narrow ordinary Dynamic Type rows.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.136 version.
- Risk: This is Models row header layout only; no download/import/delete behavior, detail sheet, untracked import, SwiftData schema, file storage, native backend, StoreKit, Mac Catalyst, Xcode project, workflow behavior, or real iPad Split View screenshot validation is changed.

### 2026-07-07

- Completed: Added a constrained-width fallback to Prompt Library template metric pills so steps/sampler/size stay horizontal when they fit and stack in narrow ordinary Dynamic Type rows.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.135 version.
- Risk: This is Prompt template metric pill layout only; no template CRUD, generation load path, Edit/Load controls, VoiceOver pill semantics, SwiftData schema, file storage, native backend, StoreKit, Mac Catalyst, Xcode project, workflow behavior, or real iPad Split View screenshot validation is changed.
- Completed: Added an accessibility Dynamic Type fallback to Plan note rows so StoreKit and Mac support notes stack the icon above long note text at large accessibility text sizes.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.134 version.
- Risk: This is Plan note row layout only; no StoreKit products, purchase UI, restore flow, receipts, entitlement persistence, paid gate, Mac Catalyst, Xcode project platform settings, native backend, SwiftData schema, workflow behavior, or real iPad/Mac window screenshot validation is changed.
- Completed: Added a width-aware title/status header fallback to Plan shared status rows so Mac readiness, capability, entitlement, and availability rows can use horizontal headers at ordinary Dynamic Type while constrained widths and accessibility text sizes keep the stacked layout.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.133 version.
- Risk: This is Plan shared status row layout only; no StoreKit products, purchase UI, restore flow, receipts, entitlement persistence, paid gate, Mac Catalyst, Xcode project platform settings, native backend, SwiftData schema, workflow behavior, or real iPad/Mac window screenshot validation is changed.
- Completed: Added a width-aware fallback to Plan Current Build and Platform Status summary rows so ordinary Dynamic Type can use horizontal title/status pairs while constrained widths and accessibility text sizes keep the stacked layout.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.132 version.
- Risk: This is Plan summary row layout only; no StoreKit products, purchase UI, restore flow, receipts, entitlement persistence, paid gate, Mac Catalyst, Xcode project platform settings, native backend, SwiftData schema, workflow behavior, or real iPad/Mac window screenshot validation is changed.
- Completed: Added a constrained-width fallback to the iPad embedded Gallery layout so the left filter rail can become a top filter shelf above the image grid.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.131 version.
- Risk: This is Gallery embedded layout selection only; no standalone Gallery split, filter state, sort state, folder actions, image grid data, detail navigation, SwiftData schema, file storage, native backend, StoreKit, Mac Catalyst, Xcode project, workflow behavior, or real iPad Split View screenshot validation is changed.
- Completed: Added vertical fixed-size behavior to Gallery tile prompt text so the existing 2-line or 3-line prompt budget can remain readable in constrained tile widths.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.130 version.
- Risk: This is Gallery tile prompt text layout only; no tile navigation, thumbnail rendering, metadata, accessibility copy, filter/sort, Gallery detail actions, SwiftData schema, file storage, native backend, StoreKit, Mac Catalyst, Xcode project, workflow behavior, or real iPad Split View screenshot validation is changed.
- Completed: Added a Gallery tile metadata width fallback so date/model metadata stays horizontal when it fits and stacks the model name in constrained tile widths.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.129 version.
- Risk: This is Gallery tile metadata layout only; no tile navigation, thumbnail rendering, prompt text, accessibility copy, filter/sort, Gallery detail actions, SwiftData schema, file storage, native backend, StoreKit, Mac Catalyst, Xcode project, workflow behavior, or real iPad Split View screenshot validation is changed.
- Completed: Added a Gallery detail preview VoiceOver hint that points users to the detail controls for reuse, regenerate, share, organize, and delete.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.128 version.
- Risk: This is Gallery detail preview accessibility hint copy only; no image rendering, Gallery actions, SwiftData schema, file storage, Generate handoff, native backend, StoreKit, Mac Catalyst, Xcode project, workflow behavior, or real VoiceOver device validation is changed.
- Completed: Added an explicit 44pt minimum hit area to the Generate result `View in Gallery` handoff button.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.127 version.
- Risk: This is Generate result Gallery handoff button layout only; no Gallery navigation, saved-record state, accessibility copy, result image preview, generation save flow, native backend, SwiftData schema, file storage, StoreKit, Mac Catalyst, Xcode project, workflow behavior, or real touch-target device validation is changed.
- Completed: Added explicit 44pt minimum hit areas to the Generate Run section actions for Generate, Cancel, Open Models, and Edit Prompt.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.126 version.
- Risk: This is Generate Run button layout only; no button action, disabled state, accessibility copy, generation gate, progress, cancel flow, result handling, native backend, SwiftData schema, file storage, StoreKit, Mac Catalyst, Xcode project, workflow behavior, or real touch-target device validation is changed.
- Completed: Added vertical fixed-size behavior to the visible Generate progress stage label so longer local render stage text can wrap instead of being compressed in narrow run panels.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.125 version.
- Risk: This is Generate progress stage text layout only; no progress fraction, cancel flow, generation gate, model selection, prompt editing, parameter editing, native backend, SwiftData schema, file storage, Gallery handoff, StoreKit, Mac Catalyst, Xcode project, workflow behavior, or real in-progress screenshot validation is changed.
- Completed: Added adaptive Generate console metrics so Steps and Canvas prefer multiple metric cards when space allows and naturally collapse toward one column in constrained widths.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.124 version.
- Risk: This is Generate console metrics grid layout only; no metric values, generation gate, model selection, prompt editing, parameter editing, native backend, SwiftData schema, file storage, Gallery handoff, StoreKit, Mac Catalyst, Xcode project, workflow behavior, or real Split View screenshot validation is changed.
- Completed: Added shared Stepper label width fallback so Steps, Width, and Height prefer compact title/value labels but can fall back vertically in constrained widths.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.123 version.
- Risk: This is shared ParameterEditor Stepper label layout selection only; no Stepper binding, range, step value, parameter normalization, generation gate, template save/load, native backend, SwiftData schema, file storage, Gallery handoff, StoreKit, Mac Catalyst, Xcode project, workflow behavior, or real Split View screenshot validation is changed.
- Completed: Added shared CFG header width fallback so the parameter header prefers compact horizontal title/value but can fall back vertically in constrained widths.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.122 version.
- Risk: This is shared ParameterEditor CFG header layout selection only; no CFG value, slider binding, parameter values, generation gate, template save/load, native backend, SwiftData schema, file storage, Gallery handoff, StoreKit, Mac Catalyst, Xcode project, workflow behavior, or real Split View screenshot validation is changed.
- Completed: Added shared parameter control width fallback so Seed/Randomize Seed and Size preset rows prefer compact horizontal controls but can fall back to readable vertical controls in constrained widths.
- Verified: Local lightweight checks, Swift parse, iPhoneOS build, and cloud CI artifact review are required for the v1.121 version.
- Risk: This is shared ParameterEditor layout selection only; no parameter values, bindings, generation gate, template save/load, native backend, SwiftData schema, file storage, Gallery handoff, StoreKit, Mac Catalyst, Xcode project, workflow behavior, or real Split View screenshot validation is changed.
