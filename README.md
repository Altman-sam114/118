...
- Hugging Face GGUF download flow with paste-and-parse Hugging Face file URLs, keyboard-aware Add Model submission, `.gguf` source validation, local GGUF file import, progress, pause, resume, cancel, confirmed deletion, duplicate protection, persisted byte tracking, untracked-file import/cleanup, restart recovery for interrupted downloads, accessible labeled row controls, readable Add Model errors, readable model list and detail status messages, and Dynamic Type-friendly model storage rows with a combined VoiceOver storage summary.
