...
- Prompt library with categories, category action menus with pointer hover affordance and category-aware VoiceOver context, accessible labeled template controls, category rename/clear actions, VoiceOver-aware empty and search-empty states, Dynamic Type-friendly prompt rows, editable saved templates with labeled prompt editors, direct saving from the generation screen, and one-click loading into the generation screen.
