...
- Plan screen that truthfully shows the current Local plan, states that StoreKit products are not configured, presents platform status, a capability matrix, entitlement rules, and Mac readiness blockers with Dynamic Type-friendly status badges and explicit VoiceOver semantics, and uses an iPad regular two-column layout without enabling purchases, entitlements, or Mac Catalyst.
